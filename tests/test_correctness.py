...
class BankerCRMTests(unittest.TestCase):
    ...
    def test_banker_outreach_template_is_present_and_current(self):
        ...
        self.assertIn("a lower-middle-market private equity firm based in Washington, D.C. and Chicago", template)
        self.assertIn("North American businesses across diversified industrials and business services", template)
        self.assertIn("particular expertise in corporate carveouts and divestitures", template)
        self.assertIn("businesses generating up to $30M of EBITDA", template)
        self.assertIn("investing $15M or more of equity", template)
        self.assertIn("up to $300M in enterprise value", template)
        self.assertIn("welcome the opportunity to work with", template)
        self.assertIn("deals in market, or coming down the pipeline", template)
        self.assertIn("I’ve attached our firm overview", template)
        self.assertNotIn("$75M to $1B in revenues", template)
        self.assertNotIn("greater than $10M in EBITDA", template)
        self.assertNotIn("transportation & mobility", template)
        self.assertNotIn("companies doing up to $30M in EBITDA", template)
        self.assertNotIn("deals in most sectors", template)
